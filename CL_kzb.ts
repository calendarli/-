...
const CL_LocalStorage = ( ( () => {
  ...
  return {
    ...
    getItem(
      key: string,
      defaultValue: any | null | undefined = undefined ): any | null | undefined {
...
const CustomBackgroundCode = CL_LocalStorage.getItem( "CustomBackgroundCode",
  "#FFF" ); // 纯色背景代码
...
( () => {
  ...
  for ( const group in GroupList ) {
    picGroupDOM.append(
      `<option value="${group}">${GroupList[ group ].Name}</option>` );
  }
  ...
  $.each( $( "select[cl-select-number]" ), ( _, item ) => {
    ...
    for ( let i = minValue; i <= maxValue; i = parseFloat(
      ( i + stepValue ).toPrecision( 12 ) ) ) {
