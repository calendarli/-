...
(function() {
    ...
    var Output = function() {
         ...
         * 获取数据
         */
        var BackgroundTransparencyNumberValue = CL_LocalStorage.getItem('BackgroundTransparencyNumberValue', '0.4') //背景透明
        var ListTransparencyNumberValue = CL_LocalStorage.getItem('ListTransparencyNumberValue', '0.6') //列表透明
        var PureColorBackgroundChecked = CL_LocalStorage.getItem('PureColorBackgroundChecked', false) //自定义纯色背景
        var CustomBackgroundCode = CL_LocalStorage.getItem('CustomBackgroundCode', '#FFF') //纯色背景代码
        var RandomPicturesChecked = CL_LocalStorage.getItem('RandomPicturesChecked', false) //随机图片
        var picGroupSelectValue = CL_LocalStorage.getItem('picGroupSelectValue', 'Anime') //分组
        var advertisingChecked = CL_LocalStorage.getItem('advertisingChecked', false) //清除广告
        var SideColumnChecked = CL_LocalStorage.getItem('SideColumnChecked', false) //删除侧栏
        var PictureURLIndex = CL_LocalStorage.getItem('PictureURLIndex', '0') //图片当前索引
        var len = GroupList[picGroupSelectValue].ImgList.length - 1 //图片索引数量
        var obj = new Image();
        /**
         * 输出背景
         */
        if (RandomPicturesChecked == true) {
            obj.src = GroupList[picGroupSelectValue].ImgList[parseInt(Math.random() * len + 1)]
        } else {
            obj.src = GroupList[picGroupSelectValue].ImgList[PictureURLIndex];
        }
        /**
         * 输出纯色背景
         */
        if (PureColorBackgroundChecked == true) {
            $('body')[0].style.backgroundColor = CustomBackgroundCode
            $('body')[0].style.backgroundImage = ''
        } else {
            obj.onload = function() {
                $('body')[0].style.backgroundImage = 'url(' + this.src + ')'
                $('body')[0].style.backgroundColor = ''
                console.log(PictureURLIndex + '/' + len)
            }
        }
        /**
         * 清除侧栏
         */
        if (SideColumnChecked == true) {
            $('div#content_right,div#rhs[jscontroller],aside[aria-label],div#right[class="right"]').hide()
        } else {
            $('div#content_right,div#rhs[jscontroller],aside[aria-label],div#right[class="right"]').show()
        }
        /**
         * 清除广告
         */
        if (advertisingChecked == true) {
            setInterval(function() { $('.new-pmd.c-container[data-ecimtimesign],div[class="result c-container"]').hide() }, 1000)
        } else {
         ...
         * 设置背景透明
         */
        $('.Masklayer')[0].style.opacity = BackgroundTransparencyNumberValue

        /**
         * 设置列表透明
         */
        $('div[class ^= result],div[data-ecimtimesign],div#rhs[jscontroller],.kp-blk.knowledge-panel.Wnoohf.OJXvsb,div.hlcw0c>.g,.ccBEnf,.MGqjK,ol#b_results li[class],#b_context li,header#b_header,#wrapper div#right').attr({ style: 'background: rgba(243 243 243 / ' + ListTransparencyNumberValue + ');backdrop-filter: blur(2px);padding: 10px 20px;border-radius: 6px;' })
    ...
    $(document).on('click', '#before', function() {
        var picGroupSelectValue = CL_LocalStorage.getItem('picGroupSelectValue') ? CL_LocalStorage.getItem('picGroupSelectValue') : 'Anime';
        var len = GroupList[picGroupSelectValue].ImgList.length - 1 //图片索引数量
        var xn = CL_LocalStorage.getItem('PictureURLIndex') ? Number(CL_LocalStorage.getItem('PictureURLIndex')) - 1 : len;
        xn = xn < '0' ? len : xn
    ...
    $(document).on('click', '#Rear', function() {
        var picGroupSelectValue = CL_LocalStorage.getItem('picGroupSelectValue') ? CL_LocalStorage.getItem('picGroupSelectValue') : 'Anime';
        var len = GroupList[picGroupSelectValue].ImgList.length - 1 //图片索引数量
        var xn = CL_LocalStorage.getItem('PictureURLIndex') ? Number(CL_LocalStorage.getItem('PictureURLIndex')) + 1 : 1;
        console.log(picGroupSelectValue + ':' + xn + '/' + len)
        xn = xn > len ? '0' : xn
        CL_LocalStorage.setItem("PictureURLIndex", String(xn))
        CL_LocalStorage.save()
            // window.location.reload();
