...
(function() {
    ...
    (function() {
        ...
        var Output = CL_LocalStorage.getItem('picGroupSelectValue') ? function() {
                /**
                 * 获取数据
                 */
                var BackgroundTransparencyNumberValue = CL_LocalStorage.getItem('BackgroundTransparencyNumberValue') //背景透明
                var ListTransparencyNumberValue = CL_LocalStorage.getItem('ListTransparencyNumberValue') //列表透明
                var PureColorBackgroundChecked = CL_LocalStorage.getItem('PureColorBackgroundChecked') //自定义纯色背景
                var CustomBackgroundCode = CL_LocalStorage.getItem('CustomBackgroundCode') //纯色背景代码
                var RandomPicturesChecked = CL_LocalStorage.getItem('RandomPicturesChecked') //随机图片
                var picGroupSelectValue = CL_LocalStorage.getItem('picGroupSelectValue') //分组
                var advertisingChecked = CL_LocalStorage.getItem('advertisingChecked') //清除广告
                var SideColumnChecked = CL_LocalStorage.getItem('SideColumnChecked') //删除侧栏
                var PictureURLIndex = CL_LocalStorage.getItem('PictureURLIndex') //图片当前索引
                ...
                $('.Masklayer')[0].style.opacity = BackgroundTransparencyNumberValue
                     ...
                     * 设置列表透明
                     */
                $('div[class ^= result],div[data-ecimtimesign]').attr({ style: 'background: rgba(243 243 243 / ' + ListTransparencyNumberValue + ');backdrop-filter: blur(2px);padding: 10px 20px;border-radius: 6px;' })
                    /**
                     * 背景前后切换
                     */
                $(document).on('click', '#before', function() {
                    var xn = Number(PictureURLIndex) - 1
                    xn = xn < '0' ? len : xn
                    console.log(xn + '/' + len)
                ...
                $('body')[0].style.backgroundImage = 'url(' + GroupList['Anime'].ImgList[0] + ')'
            }
            /**
             * 百度
             */
        $('div[class ^= result],div[data-ecimtimesign]').attr({ style: 'background: rgba(243 243 243 / 0.6);backdrop-filter: blur(2px);padding: 10px 20px;border-radius: 6px;' })
        $('div#rs,span.fk').remove()
        $('#page a,.page-inner strong').attr({ style: 'height: 36px;' })
        Output()
        ...
        $(document).on("click", "#save", function() {
             ...
             * 保存默认图片索引
             */
            PX = CL_LocalStorage.getItem("PictureURLIndex") ? function() {} : function() { CL_LocalStorage.setItem("PictureURLIndex", '0') }
            PX();
            CL_LocalStorage.save();
            // window.location.reload();
            Output()
